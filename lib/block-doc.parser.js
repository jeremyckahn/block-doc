...
var R_RETURN_DATA = /@return\s\{(.*)\}\s([\s\S]*)\*\//;
...
/**
 * @param {string} fromBlock The JSDoc block to parse description data from.
 * @return {string}
 */
...
/**
 * @param {string} fromBlock The JSDoc block to parse return data from.
 * @return {Return}
 */
function parseReturn (fromBlock) {
  var returnData = fromBlock.match(R_RETURN_DATA);

  return !returnData ? {} : {
      'type': returnData[1]
      ,'description': scrubDescriptionString(returnData[2]) || ''
    };
}
...
exports.parseMetadataList = function (text) {
  ...
  jsDocBlocks.forEach(function (block) {
    ...
    var description = scrubDescriptionString(parseDescription(block));
    var _return = parseReturn(block);
