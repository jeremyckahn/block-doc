...
function parseParams (fromBlock) {
  var params = [];
  var blockVolatileCopy = fromBlock;
  var match;
  while (parseNextParamAnnotation(blockVolatileCopy)) {
    var startFrom = blockVolatileCopy.indexOf(PARAM) + PARAM.length;
    blockVolatileCopy = blockVolatileCopy.slice(startFrom);

    var typeAndNamePair = blockVolatileCopy.match(R_NEXT_TYPE_AND_NAME_PAIR);
